...
class StyleAnalyser:
    def __init__(self, embedder: EmbedderService, registry: AuthorRegistry):
    ...
    def _analyse_embeddings(self, text: str) -> Dict[str, Any]:
        ...
        sent_embs = self._embedder.encode(sents, normalize=True)
        ...
        drift_scores = [
            1.0 - self._cosine(sent_embs[i], sent_embs[i + 1])
        ...
        if centroids:
            neutral = np.mean(list(centroids.values()), axis=0)
            neutral /= np.linalg.norm(neutral) + 1e-9
            assertiveness = 1.0 - self._cosine(doc_emb, neutral)
        else:
            assertiveness = 0.0
        ...
        return {
            ...
            "sentence_stats": {
                ...
                "mean_sentence_similarity": round(
                    float(np.mean([self._cosine(doc_emb, e) for e in sent_embs])), 4
                ),
            },
        }

    def _benchmark_authors(self, text: str) -> Dict[str, Any]:
        emb = self._embedder.encode([text], normalize=True)[0]

        centroids = self._registry.get_all_centroids()
        sims = {
            a: round(float(self._cosine(emb, c)), 4)
    ...
    def _dynamic_meta(self, text: str, emb_analysis: Dict[str, Any], author_profile: Dict[str, Any]) -> Dict[str, List[str]]:
        notes: List[str] = []
        ...
        limitations.append("Similarity scores are cosine similarities in embedding space, not probabilities.")
        limitations.append("Signals are continuous and comparative; they are not categorical emotion labels.")

        if reliability < 0.55:
            limitations.append("Short inputs reduce the stability of drift, intensity, and author-alignment signals.")
        ...
        if sims:
            ...
            probs = self._softmax(scores / 0.02)
            ...
            if len(items) >= 2:
                ...
                limitations.append("Closest-author should be interpreted as nearest semantic cluster when scores are close.")
    ...
    def analyse(self, text: str) -> Dict[str, Any]:
        ...
        emb_analysis = self._analyse_embeddings(text)
        author_profile = self._benchmark_authors(text)
        ...
        meta = self._dynamic_meta(text, emb_analysis, author_profile)
